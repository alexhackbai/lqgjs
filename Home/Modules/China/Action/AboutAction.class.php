<?php
// 本类由系统自动生成，仅供测试用途
class AboutAction extends ChinaAction {
    ...
    public function index()
    {
        ...
        // var_dump($aboutInfo);
        $aboutLs = D('News')->where('type_id = 3')->limit(3)->order('id DESC')->select();
        if (!empty($aboutLs)) {
        ...
        // var_dump($seoInfo);
        $_SEO = array();
        $_SEO['title'] = '关于我们'.'-'.$seoInfo['site_title'].'-'.$seoInfo['site_name'];
        $_SEO['kwd'] = '关于我们,'.$seoInfo['site_kwd'];
