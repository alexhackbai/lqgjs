...
class CaseAction extends ChinaAction {
    ...
    public function detail()
    {
        ...
        $hotCase = $caseDao->limit(3)->order('views DESC')->select();
        if (!empty($hotCase)) {
            for ($i=0; $i < count($hotCase); $i++) { 

                $hotCase[$i]['link'] = U('China/Case/detail', array('id'=>$hotCase[$i]['id']));
                load(extend);
                $hotCase[$i]['name'] = _msubstr(strip_tags($hotCase[$i]['name']), 0, 22);
            }
        }
        // var_dump($hotCase);
        ...
        $_SEO['kwd'] = $detailInfo['name'].',案例展示,'.$seoInfo['site_title'].','.$seoInfo['site_name'];
        $_SEO['desc'] = '案例展示,'.$detailInfo['desc'];
        // var_dump($_SEO);
        $this->assign('_SEO', $_SEO);
