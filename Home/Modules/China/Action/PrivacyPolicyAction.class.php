<?php
// 本类由系统自动生成，仅供测试用途
class PrivacyPolicyAction extends ChinaAction {
    ...
    public function index()
    {
            ...
            // var_dump($aboutInfo);
        $aboutLs = D('News')->where('type_id = 7')->order('id DESC')->select();
        if (!empty($aboutLs)) {
        ...
        // var_dump($seoInfo);
        $_SEO = array();
        $_SEO['title'] = '私有政策'.'-'.$seoInfo['site_title'].'-'.$seoInfo['site_name'];
        $_SEO['kwd'] = '私有政策,'.$seoInfo['site_kwd'];
