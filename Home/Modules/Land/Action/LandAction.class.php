<?php
// 本类由系统自动生成，仅供测试用途
class LandAction extends HomeAction {
    ...
    public function businessInfo()
    {
        ...
        if (!empty($businessInfo)) {
            for ($i=0; $i < count($businessInfo); $i++) { 
                ...
                // echo $cover['path'];
                $businessInfo[$i]['link'] = U('Land/News/detail', array('id'=>$businessInfo[$i]['id']));
                load(extend);
    ...
    public function serviceInfo()
    {
        ...
        if (!empty($serviceInfo)) {
            for ($i=0; $i < count($serviceInfo); $i++) { 
                ...
                // echo $cover['path'];
                $serviceInfo[$i]['link'] = U('Land/News/detail', array('id'=>$serviceInfo[$i]['id']));
                load(extend);
    ...
    public function serName()
    {
        ...
        if (!empty($serName)) {
            for ($i=0; $i < count($serName); $i++) {
                $serName[$i]['link'] = U('Land/Service/detail', array('id'=>$serName[$i]['id']));
            }
        }
        // var_dump($serName);
        return $serName;
