<?php
// 本类由系统自动生成，仅供测试用途
class AboutAction extends LandAction {
    ...
    public function index()
    {
        ...
        if (!empty($bandsInfo)) {
            for ($i=0; $i < count($bandsInfo); $i++) {
                $bandsInfo[$i]['link'] = U('Land/Service/detail', array('id'=>$bandsInfo[$i]['id']));
            }
        }
        // var_dump($bandsInfo);
        ...
        if (!empty($teamInfo)) {
            for ($i=0; $i < count($teamInfo); $i++) {
                ...
                $teamInfo[$i]['link'] = U('Land/About/detail', array('id'=>$teamInfo[$i]['id']));
            }
        }
        // var_dump($teamInfo);
