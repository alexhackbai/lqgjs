<?php
// 本类由系统自动生成，仅供测试用途
class SearchAction extends LandAction {
    public function result()
    {
        ...
        if (!empty($resultInfo)) {
            for ($i=0; $i < count($resultInfo); $i++) { 
                ...
                $resultInfo[$i]['link'] = U('Land/News/detail', array('id'=>$resultInfo[$i]['id']));
                load(extend);
    ...
    public function searchCase()
    {
        ...
        if (!empty($searchCaseInfo)) {
            for ($i=0; $i < count($searchCaseInfo); $i++) { 
                ...
                $searchCaseInfo[$i]['link'] = U('Land/Case/detail', array('id'=>$searchCaseInfo[$i]['id']));
            }
        }
