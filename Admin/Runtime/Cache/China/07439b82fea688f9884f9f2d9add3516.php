...
<body>
<form method="post" action="<?php echo U('China/Project/doProjectEdit');?>" enctype="multipart/form-data">
<input type="hidden" name="id" value="<?php echo ($fourInfo['id']); ?>" />
<input type="hidden" name="cover_id" value="<?php echo ($fourInfo['cover_id']); ?>" />
<table class="form">
	<tr>
		<th>产品名称：</th><td><input type="text" name="data[name]" class="ipt_s" value="<?php echo ($fourInfo['name']); ?>" /></td>
	</tr>
	<tr>
		<th>产品描述：</th><td><textarea name="data[desc]" class="text"><?php echo ($fourInfo['desc']); ?></textarea></td>
	</tr>
	<tr>
		<th>产品图片：</th><td><?php if(empty($fourInfo['cover_id'])): ?>暂无图片<?php else: ?><img src="__ROOT__<?php echo ($fourInfo['cover']); ?>" height="100" /><?php endif; ?></td>
	</tr>
	<tr>
