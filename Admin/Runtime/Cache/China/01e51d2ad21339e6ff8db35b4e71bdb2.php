...
<table class="ls">
	<tr>
		<td colspan="5"><a href="<?php echo U('China/Case/oneAdd');?>" class="btn_s">添加</a></td>
	</tr>
	<tr>
		<th width="5%">id</th><th width="10%">案例名称</th><th width="60%">案例描述</th><th width="15%">时间</th><th width="10%">操作</th>
	</tr>
	<?php if(is_array($oneInfo)): $i = 0; $__LIST__ = $oneInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		<td><?php echo ($vo["id"]); ?></td><td><?php echo ($vo["name"]); ?></td><td><?php echo ($vo["desc"]); ?></td><td><?php echo (date('Y-m-d H:i:s', $vo["ctime"])); ?></td>
		<td>
			<a href="<?php echo U('China/Case/oneEdit', array('id'=>$vo['id']));?>">编辑</a>&nbsp;
			<a href="<?php echo U('China/Case/delOne', array('id'=>$vo['id']));?>">删除</a>
		</td>
