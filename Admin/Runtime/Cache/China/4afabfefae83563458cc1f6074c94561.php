...
<table class="ls">
	<tr>
		<td colspan="5"><a href="<?php echo U('China/Ad/adAdd');?>" class="btn_s">添加</a></td>
	</tr>
	<tr>
		<th width="5%">id</th><th width="20%">广告名称</th><th width="50%">链接</th><th width="15%">时间</th><th width="10%">操作</th>
	</tr>
	<?php if(is_array($adInfo)): $i = 0; $__LIST__ = $adInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		<td><?php echo ($vo["id"]); ?></td><td><?php echo ($vo["name"]); ?></td><td><?php echo ($vo["link"]); ?></td><td><?php echo (date('Y-m-d H:i:s', $vo["ctime"])); ?></td>
		<td>
			<a href="<?php echo U('China/Ad/adEdit', array('id'=>$vo['id']));?>">编辑</a>&nbsp;
			<a href="<?php echo U('China/Ad/delAd', array('id'=>$vo['id']));?>">删除</a>
		</td>
